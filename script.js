...
// //4. when datas has been loaded from server
// xhr.onload = function () {
//   let datas = JSON.parse(this.response);
//   // for(let i=0; i<datas.length; i++){
//   //   console.log(datasFetch[i]);
//   //   var ele = document.createElement('div');
//   //   ele.innerHTML = datas[i];
...
email: "Bennett_Kreiger11@yahoo.com"
  ...
  //pagination
  var maxDatasInAPage = 10;
  var totalNoOfPages = datas.length / maxDatasInAPage;
  ...
  function createPagination(totalNoOfPages) {
    ...
    firstButt.className = "btn btn-outline-secondary btn-lg p-2 bd-highlight";
    firstButt.innerText = 'First';
    page_container.append(firstButt);

    var prevButt = document.createElement("button");
    prevButt.className = "btn btn-outline-secondary btn-lg p-2 bd-highlight";
    prevButt.innerText = 'Previous';
    page_container.append(prevButt);

    for (let p = 0; p < totalNoOfPages; p++) {
    ...
    function pageClick(page_no) {
        ...
        var end = (page_no*10)>datas.length?datas.length:page_no*10;
